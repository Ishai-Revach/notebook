...
// survives writing but not reading back means `nbk status` lies and `nbk stop`
// looks like it worked. Round-trip the awkward characters.
test('a folder path survives the round trip through the plist', () => {
  const root = '/Users/x/Notes & <Drafts>/my notebook';
  const { root: back, port } = readPlist(plist(['/usr/local/bin/node', '/x/nbk.js', 'serve', root, '--port', '4321']));
