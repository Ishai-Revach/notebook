...
- **The notebook's own code lives inside the project it serves.** Design system, shell, navigation, chrome, edit mode, tools: vendored into `<project>/notebook/` as ordinary editable files, so the project's agent can read and change any of it with no mechanism to learn. Only the kernel (daemon, write API, registry, hub switcher) lives once. See [`ARCHITECTURE.md`](ARCHITECTURE.md).
...
What leaves is the machinery: the design system, the shell, the share pipeline, the tool runtime, the server. That becomes its own product at `~/AI-Projects/notebook/`, with its own git repo, and the Meridian notebook becomes its first registered workspace.
...
```
~/AI-Projects/notebook/            ← THE OSS REPO (no content, ever)
...
<any project>/notebook/            ← A WORKSPACE: the kit, vendored, plus your content
├── notebook.json                  ← workspace config
├── NOTEBOOK.md                    ← this workspace's agent brief
...
└── .notebook/vendor.json          ← kit version + which files you have edited
```
...
```json
{
  ...
  "home": { "path": "~/AI-Projects/home-notebook", "knowledge": ["./knowledge/**"] }
}
```
...
Documents do not inline the design system; they link it, and the path resolves inside their own workspace. One change to `notebook/design-system/tokens.css` updates every document in that project, and nothing outside it. Cross-project propagation is explicit: promote the change to `~/.notebook/library/` and pull it into other workspaces, or upstream it to the kit. See [`ARCHITECTURE.md`](ARCHITECTURE.md).

### 5.2 Always on, feels like an app

Two pieces, both boring and both reliable:

1. **A launchd LaunchAgent** at `~/Library/LaunchAgents/com.ishai.notebook.plist` with `RunAtLoad` and `KeepAlive`. It starts at login, restarts on crash, survives reboot. This is how the always-on local apps you have seen do it. No Docker, no manual `npm run dev`.
...
- **Install is one command with no environment to manage.** `npx notebook-hub init` or `npm i -g notebook-hub`. Python would mean pipx or uv, a virtualenv, and a version fight on the user's machine. For a tool strangers download, that difference is most of the adoption.
...
The product is **Notebook**. The published names have to dodge two real collisions, so they differ from it on purpose:

| Thing | Name | Why |
|---|---|---|
| Product | Notebook | Your call, unchanged |
| GitHub repo | `notebook` | Repo names are namespaced by account, so no collision |
| npm package | `notebook-hub` | `notebook` and `notebook-cli` are both taken; verified 2026-07-26 |
| CLI, short | `nbk` | For typing |
| CLI, long alias | `notebook-hub` | Unambiguous, what agents and docs use |

A bare `notebook` binary is deliberately **not** claimed: Jupyter's `notebook` pip package installs a command by that name, and stepping on it would break machines belonging to exactly the technical audience most likely to try this. `nb` is also avoided (a well known notes CLI already owns it via Homebrew).

### 5.5 Distribution and install

- **Primary:** npm, global or `npx`. `npx notebook-hub@latest init` scaffolds a workspace without installing anything permanently, which is the right first taste.
...
1. **Workspace** (`<project>/notebook/library/`), this project only.
2. **User** (`~/.notebook/library/`), a source to copy from into any of your workspaces.
3. **Official** (shipped in the kit), everyone who installs Notebook.
...
- **It is per workspace,** because a client project and your home notebook should not look alike.
...
1. **The CLI is the only interface an agent needs.** `nbk` on PATH, with JSON output on every read command and exit codes that mean something. Any agent that can run a shell command can drive Notebook fully. No plugin, no protocol, no per-vendor integration.
2. **`nbk agent-brief` is the discovery primitive.** It prints the current, complete agent contract to stdout. So the instruction block that `init` writes into a project is three lines long and never goes stale: it says Notebook is here, and it says run that command to learn how to use it. The documentation lives in the tool, not in copies rotting inside a dozen `CLAUDE.md` files.
3. **`nbk init` is how an agent creates the local version.** It scaffolds the workspace, registers it, and appends its own marked instruction block to whichever agent config files the project already has. Idempotent and additive: it never overwrites what it did not write.

An MCP server (`nbk mcp`, stdio) is a thin wrapper over the same core for agents that prefer structured tools. Worth having, not worth having first.
...
| P2 | Kernel/kit split, vendoring, `nbk update` + per-document metadata and generated nav | Vendoring and the version manifest have to land before code assumes fixed locations; retrofitting a merge path later is a rewrite |
...
| P5 | `nbk` CLI, `init`, `agent-brief`, workspace guidelines file | Makes it seamless from any agent, and is the prerequisite for anyone else using it |
...
- **Total customizability turns the kit into public API,** and vendoring means every workspace can drift. The moment someone edits the shell template, its structure is a contract you cannot rename without breaking their notebook. This is the price of the promise and it is worth paying, but deliberately: a small kit, a versioned `runtime-schema`, three-way merges that never clobber, and `nbk run --safe` so a broken workspace is diagnosable in ten seconds. Full accounting in `ARCHITECTURE.md` section 9.
...
1. **Name:** Notebook. Published as `notebook-hub` on npm, binary `nbk`, repo `notebook`. See 5.4.
...
7. **Kernel and kit.** The kernel installs once (daemon, write API, registry, hub switcher, update). The kit is vendored into every workspace as editable source, so the project's agent sees the whole codebase. Upgrades come back via `nbk update`, three-way merged per file. The one locked surface is the workspace switcher. See `ARCHITECTURE.md` and `CUSTOMIZATION.md`.
...
2. **How much of the current `notebook.md` rule set becomes engine behavior** versus staying per-workspace convention. Some of it (scroll boilerplate, favicon, sidebar shell) should just be enforced by the engine so it stops being a checklist. Your style rules should not be.
